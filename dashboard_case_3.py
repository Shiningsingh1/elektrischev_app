...
def display_ev_data():
    ...
    #vis2
    value_counts = ev_user['Merk'].value_counts()
    to_remove = value_counts[value_counts < 100].index
    ev_user_count = ev_user[~ev_user.Merk.isin(to_remove)]

    fig = px.histogram(data_frame=ev_user_count, x="Merk").update_xaxes(categoryorder='total descending')
    fig.update_layout(title="Aantal elektrische voertuigen per merk")
...
def display_opencharge_map():
    ...
    #creating provinces
    nh = pd.DataFrame(data=url_data, columns=["Latitude", "Longitude", "Provincie", "Point", "Town/City",'UsageCost'])
    nh = nh.loc[nh["Provincie"] == "Noord-Holland"]

    zh = pd.DataFrame(data=url_data, columns=["Latitude", "Longitude", "Provincie", "Point", "Town/City",'UsageCost'])
    zh = zh.loc[zh["Provincie"] == "Zuid-Holland"]

    drenthe = pd.DataFrame(data=url_data, columns=["Latitude", "Longitude", "Provincie", "Point", "Town/City",'UsageCost'])
    drenthe = drenthe.loc[drenthe["Provincie"] == "Drenthe"]

    flevoland = pd.DataFrame(data=url_data, columns=["Latitude", "Longitude", "Provincie", "Point", "Town/City",'UsageCost'])
    flevoland = flevoland.loc[flevoland["Provincie"] == "Flevoland"]

    friesland = pd.DataFrame(data=url_data, columns=["Latitude", "Longitude", "Provincie", "Point", "Town/City",'UsageCost'])
    friesland = friesland.loc[friesland["Provincie"] == "Fryslân"]

    gelderland = pd.DataFrame(data=url_data, columns=["Latitude", "Longitude", "Provincie", "Point", "Town/City",'UsageCost'])
    gelderland = gelderland.loc[gelderland["Provincie"] == "Gelderland"]

    groningen = pd.DataFrame(data=url_data, columns=["Latitude", "Longitude", "Provincie", "Point", "Town/City",'UsageCost'])
    groningen = groningen.loc[groningen["Provincie"] == "Groningen"]

    limburg = pd.DataFrame(data=url_data, columns=["Latitude", "Longitude", "Provincie", "Point", "Town/City",'UsageCost'])
    limburg = limburg.loc[limburg["Provincie"] == "Limburg"]

    noordb = pd.DataFrame(data=url_data, columns=["Latitude", "Longitude", "Provincie", "Point", "Town/City",'UsageCost'])
    noordb = noordb.loc[noordb["Provincie"] == "Noord-Brabant"]

    overijssel = pd.DataFrame(data=url_data, columns=["Latitude", "Longitude", "Provincie", "Point", "Town/City",'UsageCost'])
    overijssel = overijssel.loc[overijssel["Provincie"] == "Overijssel"]

    utrecht = pd.DataFrame(data=url_data, columns=["Latitude", "Longitude", "Provincie", "Point", "Town/City",'UsageCost'])
    utrecht = utrecht.loc[utrecht["Provincie"] == "Utrecht"]

    zeeland = pd.DataFrame(data=url_data, columns=["Latitude", "Longitude", "Provincie", "Point", "Town/City",'UsageCost'])
    ...
    a = folium.FeatureGroup(name="Noord-Holland").add_to(m)
    ...
    l = folium.FeatureGroup(name="Utrecht", show=False).add_to(m)
...
def display_charge_data():
    ...
    #vis3
    fig = px.scatter(lp_user, x="ConnectedTime_min", y="ChargeTime_min", color="Month", size="MaxPower",
                labels={"ChargeTime_min": "Charge time (min)",
                       "ConnectedTime_min": "Connected time (min)"},
                title= "Charge time ten opzichten van de Connected time")
    st.plotly_chart(fig)
